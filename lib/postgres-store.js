...
import { createAccessToken, createPasswordRecord, verifyPassword } from "./security.js";
...
export class PostgresStore {
  ...
  async resolveToken(token) {
    const sql = `
      SELECT json_build_object('role', role, 'userId', user_id)
      FROM api_tokens
      WHERE token = ${sqlString(token)} AND is_active = TRUE;
    `;
    return this.runJsonQuery(sql);
  }
  ...
  async signup(payload) {
    ...
    const existing = this.runJsonQuery(`SELECT id FROM platform_users WHERE email = ${sqlString(payload.email)}`);
    if (existing) {
    ...
    const sql = `
      WITH id_gen AS (
        SELECT COALESCE(MAX(NULLIF(SUBSTRING(id FROM '[0-9]+$'), '')::int), 0) + 1 as next_id
        FROM platform_users
        WHERE id LIKE '${userIdPrefix}_%'
      ),
      inserted_user AS (
        INSERT INTO platform_users (
          id, role, full_name, phone, email, password_hash, password_salt, wallet_balance, online_status
        )
        SELECT
          '${userIdPrefix}_' || LPAD((SELECT next_id FROM id_gen)::text, 3, '0'),
          ${sqlString(payload.role)},
          ${sqlString(payload.fullName)},
          ${sqlString(payload.phone)},
          ${sqlString(payload.email)},
          ${sqlString(passwordRecord.hash)},
          ${sqlString(passwordRecord.salt)},
          ${payload.role === "rider" ? "0" : "NULL"},
          ${payload.role === "courier" || payload.role === "driver" ? "'offline'" : "NULL"}
        RETURNING id, role, full_name, email
      ),
      inserted_token AS (
        INSERT INTO api_tokens (token, user_id, role, label)
        SELECT ${sqlString(token)}, id, role, 'Signup token'
        FROM inserted_user
        RETURNING token
      )
      SELECT json_build_object(
        'token', (SELECT token FROM inserted_token),
        'user', (SELECT json_build_object('userId', id, 'role', role, 'email', email, 'fullName', full_name) FROM inserted_user)
      );
    `;

    try {
      const result = this.runJsonQuery(sql);
      return { statusCode: 201, payload: result };
    } catch (err) {
      return { statusCode: 500, payload: { error: "Failed to create user" } };
    }
  }
  ...
  async login(payload) {
    ...
    const user = this.runJsonQuery(`
      SELECT id, role, email, full_name as "fullName", password_hash as "passwordHash", password_salt as "passwordSalt"
      FROM platform_users
      WHERE email = ${sqlString(payload.email)}
    `);
    ...
    this.runCommand(`
      INSERT INTO api_tokens (token, user_id, role, label)
      VALUES (${sqlString(token)}, ${sqlString(user.id)}, ${sqlString(user.role)}, 'Login token');
    `);
  ...
  async getMarketplaceCategories() {
    return this.runJsonQuery(`
      SELECT json_build_object(
        'categories', COALESCE((SELECT json_agg(json_build_object('id', id, 'name', name, 'description', description, 'status', status) ORDER BY name) FROM business_categories), '[]'::json),
        'cities', COALESCE((SELECT json_agg(json_build_object('id', id, 'name', name, 'country', country, 'center', json_build_object('lat', center_lat, 'lng', center_lng), 'status', status) ORDER BY name) FROM cities), '[]'::json)
      );
    `);
  }

  async getMarketplaceBusinesses(query = {}) {
    const q = `%${String(query.q || "").replaceAll("'", "''").toLowerCase()}%`;
    const city = `%${String(query.city || "").replaceAll("'", "''").toLowerCase()}%`;
    const category = String(query.category || "").replaceAll("'", "''").toLowerCase();
    const minRating = Number(query.rating || 0);
    const openNow = ["true", "1", "yes"].includes(String(query.openNow || "").toLowerCase());
    const serviceMode = `%${String(query.serviceMode || "").replaceAll("'", "''").toLowerCase()}%`;

    return this.runJsonQuery(`
      WITH filtered AS (
        SELECT b.*
        FROM marketplace_businesses b
        JOIN business_categories bc ON bc.id = b.category_id
        JOIN cities c ON c.id = b.city_id
        WHERE b.status = 'published'
          AND (${sqlString(category)} = '' OR LOWER(b.category_id) = ${sqlString(category)} OR LOWER(bc.name) LIKE ${sqlString(`%${category}%`)})
          AND (${sqlString(query.q || "")} = '' OR LOWER(CONCAT_WS(' ', b.name, b.description, b.address, b.neighborhood, bc.name, c.name, c.country)) LIKE ${sqlString(q)})
          AND (${sqlString(query.city || "")} = '' OR LOWER(CONCAT_WS(' ', c.name, c.country, b.neighborhood, b.address)) LIKE ${sqlString(city)})
          AND (${openNow ? "b.is_open = TRUE" : "TRUE"})
          AND b.rating >= ${sqlNumeric(minRating, 0)}
          AND (${sqlString(query.serviceMode || "")} = '' OR EXISTS (
            SELECT 1 FROM business_service_modes bsm WHERE bsm.business_id = b.id AND LOWER(bsm.service_mode) LIKE ${sqlString(serviceMode)}
          ))
      )
      SELECT json_build_object(
        'businesses', COALESCE((SELECT json_agg(json_build_object(
          'id', b.id,
          'slug', b.slug,
          'name', b.name,
          ...
          'status', b.status,
          'category', json_build_object('id', bc.id, 'name', bc.name, 'description', bc.description),
          'city', json_build_object('id', c.id, 'name', c.name, 'country', c.country, 'center', json_build_object('lat', c.center_lat, 'lng', c.center_lng)),
          'tags', COALESCE((SELECT json_agg(tag ORDER BY tag) FROM business_tags WHERE business_id = b.id), '[]'::json),
          'serviceModes', COALESCE((SELECT json_agg(service_mode ORDER BY service_mode) FROM business_service_modes WHERE business_id = b.id), '[]'::json),
          'trustBadges', COALESCE((SELECT json_agg(badge ORDER BY badge) FROM business_trust_badges WHERE business_id = b.id), '[]'::json),
          'topServices', COALESCE((SELECT json_agg(json_build_object('name', name, 'description', description, 'priceLabel', price_label) ORDER BY sort_order) FROM business_services WHERE business_id = b.id), '[]'::json)
        ) ORDER BY b.rating DESC) FROM filtered b JOIN business_categories bc ON bc.id = b.category_id JOIN cities c ON c.id = b.city_id), '[]'::json),
        'categories', COALESCE((SELECT json_agg(json_build_object('id', id, 'name', name, 'description', description, 'status', status) ORDER BY name) FROM business_categories), '[]'::json),
        'cities', COALESCE((SELECT json_agg(json_build_object('id', id, 'name', name, 'country', country, 'center', json_build_object('lat', center_lat, 'lng', center_lng), 'status', status) ORDER BY name) FROM cities), '[]'::json)
      );
    `);
    ...
    return this.runJsonQuery(`
      SELECT json_build_object(
        'business', json_build_object(
          ...
          'trustBadges', COALESCE((SELECT json_agg(badge ORDER BY badge) FROM business_trust_badges WHERE business_id = b.id), '[]'::json),
          'topServices', COALESCE((SELECT json_agg(json_build_object('name', name, 'description', description, 'priceLabel', price_label) ORDER BY sort_order) FROM business_services WHERE business_id = b.id), '[]'::json),
          'hours', COALESCE((SELECT json_object_agg(weekday, hours_label) FROM business_hours WHERE business_id = b.id), '{}'::json)
        ),
        'reviews', COALESCE((SELECT json_agg(json_build_object('id', id, 'businessId', business_id, 'userId', user_id, 'rating', rating, 'title', title, 'body', body, 'verificationSource', verification_source, 'status', status, 'createdAt', created_at) ORDER BY created_at DESC) FROM marketplace_reviews WHERE business_id = b.id AND status = 'published'), '[]'::json)
      )
      FROM marketplace_businesses b
      JOIN business_categories bc ON bc.id = b.category_id
      JOIN cities c ON c.id = b.city_id
      WHERE (b.slug = ${sqlString(slug)} OR b.id = ${sqlString(slug)}) AND b.status = 'published'
      LIMIT 1;
    `);
  }
  ...
  async getMarketplaceReviews(businessId) {
    return this.runJsonQuery(`
      SELECT json_build_object(
        'reviews', COALESCE((SELECT json_agg(json_build_object('id', id, 'businessId', business_id, 'userId', user_id, 'rating', rating, 'title', title, 'body', body, 'verificationSource', verification_source, 'status', status, 'createdAt', created_at) ORDER BY created_at DESC) FROM marketplace_reviews WHERE business_id = ${sqlString(businessId)} AND status = 'published'), '[]'::json)
      );
    `);
  }
